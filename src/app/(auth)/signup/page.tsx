 ...
 * It's wrapped in a component that ensures it's only rendered on the client.
 */
function SignupForm() {
...
// This is the "guard" component that safely handles client-side rendering.
function SignupPageContent() {
    const [isClient, setIsClient] = useState(false);

    // This hook ensures that the component only renders the real form on the client side,
    // which is necessary to prevent hydration errors with browser-only scripts like PayPal.
    useEffect(() => {
        setIsClient(true);
    }, []);

    // While waiting for the client to mount, show a skeleton.
    if (!isClient) {
        return <SignupLoadingSkeleton />;
    }

    // Once the client has mounted, render the real form.
    return <SignupForm />;
}

// The main page export. It wraps the content in Suspense to safely handle useSearchParams.
